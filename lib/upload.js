...
function main () {
  ...
  AWS.config.credentials.accessKeyId = getEnvVariable('AWS_ACCESS_KEY_ID');
  AWS.config.credentials.secretAccessKey = getEnvVariable('AWS_SECRET_ACCESS_KEY');
  ...
  glob(PUBLIC_ASSETS_SOURCE_DIRECTORY + '/**/*.*', {}, function(error, files) {
      ...
      async.eachLimit(files, 16, function(file, callback) {
          ...
          const contentType = mimeTypes.lookup(path.extname(file)) || null;
